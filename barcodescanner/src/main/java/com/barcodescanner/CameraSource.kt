 ...
 */
private const val REQUESTEDFPS = 20.0f
private const val REQUESTEDPREVIEWWIDTH = 480
private const val REQUESTEDPREVIEWHEIGHT = 360
private const val REQUESTEDAUTOFOCUS = true

private const val NI = 90
private const val ONE_EIGHTY = 180
private const val TWO_SEVENTY = 270
private const val THREE_SIXTY = 360
private const val EIGHT_POINT_ZERO = 8.0
private const val THOUSAND = 1000.0f

@SuppressLint("MissingPermission")
class CameraSource(protected var activity: Context, private val scanResultListener: ScanResultListener) {
    ...
    fun createCameraSplit(camera: Camera): Camera {
    ...
    private fun createCamera(): Camera {
        ...
        var camera = Camera.open(requestedCameraId)

        camera = createCameraSplit(camera)
    ...
    private fun setRotation(camera: Camera, parameters: Camera.Parameters, cameraId: Int) {
        ...
        when (rotation) {
            ...
            Surface.ROTATION_90 -> degrees = NI
            Surface.ROTATION_180 -> degrees = ONE_EIGHTY
            Surface.ROTATION_270 -> degrees = TWO_SEVENTY
        ...
        if (cameraInfo.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
            angle = (cameraInfo.orientation + degrees) % THREE_SIXTY
            displayAngle = (THREE_SIXTY - angle) % THREE_SIXTY // compensate for it being mirrored
        } else { // back-facing
            angle = (cameraInfo.orientation - degrees + THREE_SIXTY) % THREE_SIXTY
            displayAngle = angle
        }

        // This corresponds to the rotation constants.
        this.rotation = angle / NI
    ...
    private fun createPreviewBuffer(previewSize: Size): ByteArray {
        ...
        val sizeInBits = previewSize.getHeight().toLong() * previewSize.getWidth().toLong() * bitsPerPixel.toLong()
        val bufferSize = Math.ceil(sizeInBits / EIGHT_POINT_ZERO).toInt() + 1
    ...
    companion object {
        ...
        private fun selectPreviewFpsRange(camera: Camera, desiredPreviewFps: Float): IntArray? {
            ...
            // rates.
            val desiredPreviewFpsScaled = (desiredPreviewFps * THOUSAND).toInt()
